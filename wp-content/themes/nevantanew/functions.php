    ...
    for ($i = 0; $i < $totalitems; $i++) {
        ...
        if ($items[$i]->menu_item_parent == 0) {
            ...
            if ($i == 0) {
                ...
                $sstring.="<li class='dropdown ".$active." '><a href='" . $items[$i]->url . "' class='dropdown-toggle' role='button' aria-haspopup='true' aria-expanded='false'>Fashion</a>";
            } else {
                ...
                if ($items[$i + 1]->menu_item_parent == 0) {
                    ...
                    if(($items[$i]->title == 'Blog') && ($isItBlog!==false)){
                        $active = 'active';
                    }else { $active = ''; }
                    $sstring.="<li class='dropdown ".$active."' ><a href='" . $items[$i]->url . "' class='dropdown-toggle'>" . $items[$i]->title . "</a>";
                }
            }
        } else {
            if ($flag == 1) {
...
$label_gallery = array(
        'name'              => _x( 'Gallery Seasons', 'taxonomy general name' ),
        'singular_name'     => _x( 'Gallery Season', 'taxonomy singular name' ),
        'search_items'      => __( 'Gallery Season' ),
        'all_items'         => __( 'All Gallery Season' ),
        'parent_item'       => __( 'Parent Gallery Season' ),
        'parent_item_colon' => __( 'Parent Gallery Season:' ),
        'edit_item'         => __( 'Edit Gallery Season' ),
        'update_item'       => __( 'Update Gallery Season' ),
        'add_new_item'      => __( 'Add New Gallery Season' ),
        'new_item_name'     => __( 'New Gallery Season' ),
        'menu_name'         => __( 'Gallery Season' ),
    );
$args = array(
        'hierarchical'      => true,
        'labels'            => $label_gallery,
        'show_ui'           => true,
        'show_admin_column' => true,
        'query_var'         => true,
        'rewrite'               => array( 'slug' => 'gallery_season' ),
    );

    register_taxonomy( 'gallery_season', array( 'post' ), $args );
...
if ( function_exists( 'add_theme_support' ) ) {
	add_theme_support( 'post-thumbnails' );
	//add_image_size( 'post-thumb-first', 136, 145, array( 'center', 'top' ) ); // larghezza 300 pixel (e altezza illimitata) //not using
	...
	//add_image_size( 'blog-thumb-nine', 250, 200, array( 'center', 'top' )); //not using			
	//add_image_size( 'blog-thumb-tenth', 350, 526, array( 'center', 'top' ) );
	//add_image_size( 'blog-thumb-eleven', 1140, 631, array( 'center', 'top' ) );
	
	//add_image_size( 'blog-thumb-twelve', 1295, 717, array( 'center', 'top' ) );//not using
	//add_image_size( 'blog-thumb-thirteen', 220, 220, array( 'center', 'top' ) );
	//add_image_size( 'blog-thumb-fourteen', 260, 260, array( 'center', 'top' ) ); //not using
	...
	//add_image_size( 'blog-thumb-eighteen', 150, 150, array( 'center', 'top' ) ); //not using
	//add_image_size( 'blog-thumb-nineteen', 198, 213, array( 'center', 'top' ) );//not using
    ...
    add_image_size( 'gallery-thumb-images', 500 , 500 , false );
}
...
add_shortcode('tag_video_blog', 'get_tag_video_blog');

function get_tag_video_blog($atts) {
    $tag = get_queried_object();
    ...
    $video_args = array(
        'orderby' => 'post_date',
        'order' => 'DESC',
        'posts_per_page' => $count_to_display,
        'post_type' => 'post',
        'tax_query' => array(
            array(
                'taxonomy' => 'post_tag',
    ...
    $total_tag_video = $get_video_query->found_posts;
    $getallvideos = $get_video_query->posts;
    $blog_args = array(
        'orderby' => 'post_date',
        'order' => 'DESC',
        'posts_per_page' => $count_to_display,
    ...
    $get_blog_query = new WP_Query($blog_args);
    $total_tag_blog = $get_blog_query->found_posts;
    $getallblogs = $get_blog_query->posts;
?>
    <?php if ($get_video_query->have_posts()): ?>
    <div class="container">  
    ...
    wp_reset_postdata();
        ?>
    <?php if ($get_blog_query->have_posts()): ?>
    <div class="container">  
    <div class="tag_video_new">  BLOG  </div>
        <?php if ($total_tag_blog > $count_to_display) { ?>
    <div class="View-More">  <a  id="blog_view_more">
    <form method="Post" action="" id="blog_tag_form">
    <input type="hidden" value="blog" name="show_type">
     </form>View More</a> </div> 
        <?php } ?>
    <div class="col-md-12 col-sm-12 header-bot-line nopad-left-right"><hr></div>
    <div class="row mob_tag_top">
    <div class="col-md-12 nopad-left-right">
    ...
    $image_data = wp_get_attachment_image_src(get_post_thumbnail_id($blog->ID), 'blog-thumb-twentythree');
    $imagesrc = $image_data[0];
    if (is_url_exist($imagesrc)) {
    ?>
    <div class="col-md-3">
...
add_shortcode('tag_list_all', 'get_tag_all_posts');

function get_tag_all_posts($atts) {
    $post_type = $atts['show_type'];
    $tag_title = ($post_type == 'post' ? "VIDEOS" : "BLOG");
    $tag = get_queried_object();
    $term_id = $tag->term_id;
    $tag_name = strtolower($tag->name);
    ...
    if (isset($posts_array)) {
        ...
        ?>
        <div class="container">  
        <div class="tag_video_new">  <?php echo $tag_title; ?></div> 
        <div class="View-More"><a href="<?php echo $current_url; ?>">View Less</a>
        </div>
        <div class="col-md-12 col-sm-12 header-bot-line nopad-left-right"><hr></div>
        <div class="row">
        <div class="col-md-12 nopad-left-right">
        ...
        <div class="innerpage-video-img">
        <img class="img-res" src="<?php echo $imagesrc; ?>" width="272" height="153">
         <?php if ($tag_title == 'VIDEOS') {?>
        <div class="video-play-btn-inner-page">
        <img src="/wp-content/themes/nevantanew/img/play.svg" class="play-inner-page">         </div><?php } ?>
        </div>                         
        <div class="title-post-inner-page">
...
add_shortcode('tag_video_blog_mobile', 'get_tag_video_blog_mobile');

function get_tag_video_blog_mobile($atts) {
    $tag = get_queried_object();
    $tag_name = strtolower($tag->name);
    $term_id = $tag->term_id;
    $count_to_display = 4;
...
?>
    ...
    ?>
    <div class="videolist col-50 mobi-ply">
                <a href="<?php echo get_permalink($blog->ID); ?>" class="fashion">         
                    <amp-img layout="responsive" src="<?php echo $imagesrc; ?>" width="208" height="117" alt="<?php the_title(); ?>"></amp-img>
                </a>
                 <div class="title_post_mobile">
                <a href="<?php echo get_permalink($blog->ID); ?>">
                <?php echo $blog->post_title; ?></a>
                </div>
                </div>
...
add_shortcode('tag_list_all_mobile', 'get_tag_all_posts_mobile');

function get_tag_all_posts_mobile($atts) {
    $post_type = $atts['show_type'];
    $tag_title = ($post_type == 'post' ? "VIDEOS" : "BLOG");
    $tag = get_queried_object();
    $term_id = $tag->term_id;
            ...
            if (is_url_exist($imagesrc)) { ?>
             <div class="videolist col-50 mobi-ply">
                <a href="<?php echo get_permalink($post->ID); ?>" class="fashion">         
                    <amp-img layout="responsive" src="<?php echo $imagesrc; ?>" width="208" height="117" alt="<?php the_title(); ?>"></amp-img>
                    <?php if ($tag_title == 'VIDEOS') {?>
                     <amp-img layout="responsive" class="video-play-btn-inner-page-mobile" height="150" width="150"  src="<?php bloginfo('template_directory'); ?>/amp-nevanta/img/play-mobile.svg" alt="Nevanta"></amp-img> <?php } ?>
                </a>
                 <div class="title_post_mobile">
                <a href="<?php echo get_permalink($post->ID); ?>">
                <?php echo $post->post_title; ?>
